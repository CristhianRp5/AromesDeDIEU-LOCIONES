...
class QRService {
    ...
    createVerificationURL(qrId, productId) {
        // Usar la configuración del dominio establecida en el admin panel
        if (window.QR_CONFIG && window.QR_CONFIG.generateVerificationURL) {
            return window.QR_CONFIG.generateVerificationURL(qrId, null);
        }
        
        // Fallback a configuración manual del dominio
        const domain = 'https://www.aromesdedieu.com';
        return `${domain}/verificacion-qr.html?qr=${qrId}&id=${productId}&brand=AROME-DE-DIEU`;
    }
