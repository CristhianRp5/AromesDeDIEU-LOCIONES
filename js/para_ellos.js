...
class ParaEllosManager {
    ...
    updatePriceFilter() {
        ...
        if (simplePriceFilters) {
            this.initializeSimplePriceFilters();
        } else {
    ...
    initializeSimplePriceFilters() {
        console.log('🎨 Inicializando filtros de precio simples');
        
        // Elementos de los filtros simples
        ...
        if (!minPriceBtn || !maxPriceBtn || !discountBtn) {
            console.warn('⚠️ No se encontraron los botones de filtro simple');
            return;
        }
        ...
        minPriceBtn.addEventListener('click', () => {
            console.log('🖱️ Click en botón Precio Mínimo');
            this.clearActiveFilters();
            minPriceBtn.classList.add('active');
            this.applySortFilter('price_asc');
        });

        maxPriceBtn.addEventListener('click', () => {
            console.log('🖱️ Click en botón Precio Máximo');
            this.clearActiveFilters();
            maxPriceBtn.classList.add('active');
            this.applySortFilter('price_desc');
        });

        discountBtn.addEventListener('click', () => {
            console.log('🖱️ Click en botón Con Descuento');
            this.clearActiveFilters();
        ...
        // Event listener para toggle del filtro
        if (toggleFilterBtn && filterButtonsContainer && simplePriceFilters) {
            toggleFilterBtn.addEventListener('click', () => {
                console.log('🔄 Toggle filtros');
                const isCollapsed = simplePriceFilters.classList.contains('collapsed');
                
                simplePriceFilters.classList.toggle('collapsed');
                
                // Cambiar el ícono del botón según el estado
                const svg = toggleFilterBtn.querySelector('svg polyline');
                if (isCollapsed) {
                    // Si estaba colapsado, ahora se abre (flecha hacia abajo)
                    svg.setAttribute('points', '6,9 12,15 18,9');
                    console.log('📂 Filtros abiertos');
                } else {
                    // Si estaba abierto, ahora se cierra (flecha hacia la derecha)
                    svg.setAttribute('points', '9,18 15,12 9,6');
                    console.log('📁 Filtros cerrados');
                }
            });
            
            // Verificar estado inicial y asegurar que el ícono sea correcto
            if (simplePriceFilters.classList.contains('collapsed')) {
                const svg = toggleFilterBtn.querySelector('svg polyline');
                svg.setAttribute('points', '9,18 15,12 9,6'); // Flecha derecha para estado cerrado
                console.log('🔒 Filtros iniciados en estado cerrado');
            }
        }

        console.log('✅ Filtros de precio simples inicializados');
    }

    clearActiveFilters() {
        const buttons = document.querySelectorAll('.simple-filter-btn');
        buttons.forEach(btn => btn.classList.remove('active'));
    }

    clearAllFilters() {
        console.log('🧹 Limpiando todos los filtros activos');
    ...
    applyDiscountFilter() {
        console.log('🔄 Aplicando filtro de descuento/oferta');
        ...
        if (this.filteredProducts.length === 0) {
            console.warn('⚠️ No se encontraron productos con descuento/oferta');
